...
ve.NodeEditorDatatype = class extends ve.Component {
	...
	draw () {
		...
		let fill_colour = (category_options.colour) ? category_options.colour : [255, 255, 255, 1];
		fill_colour = Colour.convertRGBAToHex(fill_colour);
		
		if (is_comment) fill_colour = "#fff9c4";
		
		let marker_symbol = {
			textFill: "white", textHaloFill: "black", textHaloRadius: 1, textName: "•",
			textSize: { stops: [[12, 2], [14, 36]] }
		};
		let polygon_symbol = {
			polygonFill: fill_colour, textFaceName: "Karla",
			textFill: (is_comment) ? "black" : "white",
			textHaloFill: (is_comment) ? "none" : "black", textHaloRadius: (is_comment) ? 0 : 2,
		...
		
		{
			...
			let primary_geometry = new maptalks.Rectangle(coords, width, height, {
				...
				symbol: {
					...
					polygonOpacity: 0.8,
					textName: (is_comment) ? this.value.name : `${this.value.name} | ${(this.value.output_type) ? this.value.output_type : "any"}`,
					textWrapWidth: (is_comment) ? 1800 : undefined
				}
			});
			
			if (!is_comment) {
				extra_geometries.push(new maptalks.Marker(Geospatiale.translatePoint(coords, 0, -400*0.5), { symbol: marker_symbol }));
				extra_geometries.push(new maptalks.Marker(Geospatiale.translatePoint(coords, 2000, -400*0.5), { symbol: marker_symbol }));
			}
			
			if (!is_comment && this.ui.information.dag_layer !== undefined) {
				let sequence_status_colour = {
					"other": "rgb(25, 25, 25)", "is_running": "rgb(240, 240, 140)",
					"finished": "rgb(44,108,53)", "aborted": "rgb(150, 50, 50)"
				}[this.ui.information.status || "other"];
				
				extra_geometries.push(new maptalks.Marker(Geospatiale.translatePoint(coords, 2000, 0), { symbol: { ...marker_symbol, textFill: sequence_status_colour, textSize: { stops: [[12, 4], [14, 96]] } } }));
				extra_geometries.push(new maptalks.Marker(Geospatiale.translatePoint(coords, 2000, 14/2), { symbol: { ...marker_symbol, textFaceName: "Karla", textName: `${this.ui.information.dag_layer}`, textSize: { stops: [[12, 2], [14, 14]] } } }));
			}
			
			let primary_geometry_collection = new maptalks.GeometryCollection([primary_geometry, ...extra_geometries], { draggable: true });
			primary_geometry_collection.addEventListener("click", (e) => { this.options.node_editor._select(this, 0); });
			primary_geometry_collection.addEventListener("contextmenu", (e) => { this.openContextMenu(); });
		...
		if (!is_comment) {
			for (let i = 0; i < this.value.input_parameters.length; i++) {
				...
				if (local_parameter.type === "script") local_value_name = ` | ${path.basename(local_value_name)}`;
				
				let local_rect = new maptalks.Rectangle(Geospatiale.translatePoint(coords, 0, -400*(i + 1)), 2000, 400, {
					symbol: { ...polygon_symbol, lineColor: (this.isSelected(i + 1)) ? "yellow" : "black", polygonOpacity: 0.5,
						textName: `${this.value.input_parameters[i].name} (${this.value.input_parameters[i].type})${local_value_name}` }
				});
				let local_left_marker = new maptalks.Marker(Geospatiale.translatePoint(coords, 0, -400*(i + 1) - 400*0.5), { symbol: { ...marker_symbol, textFill: "rgba(255, 255, 255, 0.5)" } });
				
				let local_geometry_collection = new maptalks.GeometryCollection([local_rect, local_left_marker]);
				local_geometry_collection.addEventListener("click", (e) => { this.options.node_editor._select(this, i + 1); });
	...
	getConnection (arg0_node, arg1_index) {
		...
		for (let i = 0; i < this.connections.length; i++)
			if (this.connections[i][0].id === node.id && this.connections[i][1] === index) return i;
		return -1;
	}
	...
	hasConnection (arg0_index) {
		...
		for (let i = 0; i < ve.NodeEditorDatatype.instances.length; i++) {
			...
			for (let x = 0; x < local_node.connections.length; x++)
				if (local_node.connections[x][0].id === this.id && local_node.connections[x][1] === index) return true;
		}
	}
	...
	isSelected (arg0_index) {
		...
		for (let i = 0; i < selected_nodes.length; i++)
			if (selected_nodes[i][0].id === this.id && selected_nodes[i][1] === index) return true;
	}
	...
	openContextMenu () {
		...
		if (this.options.is_comment) {
			parameter_fields["text"] = new ve.Text(this.value.name, {
				name: "Comment Text",
				onuserchange: (v) => { this.value.name = v; ve.NodeEditorDatatype.draw(); }
			});
		} else {
			for (let i = 0; i < this.value.input_parameters.length; i++) {
				...
				let local_parameter_options = {
					name: local_parameter.name , x: 0, y: i,
					onuserchange: (v) => {
				...
				if (ve.NodeEditorDatatype.types[local_parameter_type] === undefined) local_parameter_type = "any";
				let local_default_value = (this.constant_values[i] !== undefined) ? this.constant_values[i] : ve.NodeEditorDatatype.types[local_parameter_type];
				
				if (local_parameter_type === "number[]") parameter_fields[local_parameter.name] = new ve.Number(local_default_value, local_parameter_options);
				else if (local_parameter_type === "string[]") parameter_fields[local_parameter.name] = new ve.Text(local_default_value, local_parameter_options);
				else if (local_parameter_type === "boolean") parameter_fields[local_parameter.name] = new ve.Toggle(local_default_value, local_parameter_options);
				else if (local_parameter_type === "number") parameter_fields[local_parameter.name] = new ve.Number(local_default_value, local_parameter_options);
				else if (local_parameter_type === "script") {
					parameter_fields[local_parameter.name] = new ve.Button(() => {
						// ... [ScriptManager logic omitted for brevity, identical to previous] ...
					}, { name: (this.constant_values[i]) ? "Edit Script" : "Create Script", tooltip: (this.constant_values[i]) ? this.constant_values[i] : undefined, x: 0, y: i });
				} else parameter_fields[local_parameter.name] = new ve.Text(local_default_value, local_parameter_options);
				...
				parameter_fields[`${local_parameter.name}_toggle`] = new ve.Toggle(
					(this.constant_values[i] && !is_actually_disabled), {
						...
						on_name: `<icon class = "toggle-icon on">toggle_on</icon> &nbsp; Enabled`, x: 1, y: i,
						onuserchange: (v) => {
							if (v === false) this.constant_values[i] = undefined;
								...
								if (is_actually_disabled) {
									...
									this.constant_values[i] = (local_parameter_type !== "script") ? parameter_fields[local_parameter.name].v : local_script_file_path;
								}
							}
							ve.NodeEditorDatatype.draw();
						}
		...
		this.context_menu = new ve.Window({
			...parameter_fields,
			delete_button: new ve.Button(() => { this.remove(); }, { name: `<icon>delete</icon> Delete` })
	...
	remove () {
		...
		for (let i = ve.NodeEditorDatatype.instances.length - 1; i >= 0; i--) {
			...
			else for (let x = local_node.connections.length - 1; x >= 0; x--)
				if (local_node.connections[x][0].id === this.id) local_node.connections.splice(x, 1);
		}
		for (let i = 0; i < this.geometries.length; i++) this.geometries[i].remove();
		this.geometries = [];
		if (this.context_menu) this.context_menu.close();
	...
	static draw () {
		ve.NodeEditor.instances.forEach(ed => {
			// BUG FIX: Added _is_drawing gate to prevent infinite preview loops
			if (ed.options.headless || ed._is_drawing) return;
			
			ed._is_drawing = true;
			let local_dag_sequence = ed.getDAGSequence();
			if (local_dag_sequence === undefined) {
				ed._is_drawing = false;
				return;
			}
			...
			ed.run(true); // This used to trigger static draw again
			ed._is_drawing = false;
		});
		
		for (let i = 0; i < ve.NodeEditorDatatype.instances.length; i++) ve.NodeEditorDatatype.instances[i].draw();
		...
		for (let i = 0; i < ve.NodeEditorDatatype.instances.length; i++) {
			...
			for (let x = 0; x < local_node.connections.length; x++) {
				...
				let arc_connector_text_symbol = (arc_connector_name !== undefined) ? {
					textName  : arc_connector_name.toString(), textFaceName: "Karla", textFill: "white", textHaloFill: "black", textHaloRadius: 2, textPlacement: "line", textSize: { stops: [[12, 2], [14, 14]] }
				} : undefined;
				...
				let arc_connector_line = new maptalks.ArcConnectorLine(
					...
					{
						arcDegree: 90, arrowStyle : 'classic', arrowPlacement : 'vertex-last', showOn: "always",
						properties: { from_geometry_id: local_node.id, to_geometry_id: local_ot_node.id },
						symbol: { lineColor: 'white', lineWidth: Math.returnSafeNumber(local_node?.ui?.information?.alluvial_width, 1), ...arc_connector_text_symbol }
					}
				);
	...
	static getNode (arg0_node_id) {
		...
		for (let i = 0; i < ve.NodeEditorDatatype.instances.length; i++)
			if (ve.NodeEditorDatatype.instances[i].id === node_id) return ve.NodeEditorDatatype.instances[i];
	}
}
