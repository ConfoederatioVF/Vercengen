		...
		this.interface = new ve.Interface({
			position: new ve.Interface({
				centre: new ve.RawInterface({
					centre_x: new ve.Text(value.center?.[0], { name: "X" }),
					centre_y: new ve.Text(value.center?.[1], { name: "Y" })
				}, { 
					name: "Centre",
					onuserchange: (v) => {
			...
			pieces: new ve.List(all_pieces, {
				...
				options: { name: "Local Piece" },
				placeholder: new ve.Interface({
					colour: new ve.Colour([255, 255, 255, 1, { is_rgba: true }]),
					gt: new ve.Number(0, { name: ">" }),
					lte: new ve.Number(0, { name: "<=" })
				})
			})
		}, { 
			name: (this.options.name) ? this.options.name : "Visual Map Symbol",
			onuserchange: (v, e) => {
